...
export function addSyntheticLeadingComment<T extends Node>(
    ...
    emitNode.leadingComments = [...emitNode.leadingComments ?? [], { kind, text, hasTrailingNewLine }];
    return node;
}
...
export function addSyntheticTrailingComment<T extends Node>(
    ...
    const emitNode = getOrCreateEmitNode(node);
    emitNode.trailingComments = [...emitNode.trailingComments ?? [], { kind, text, hasTrailingNewLine }];
    return node;
}
...
export function getEmitNode(node: Node): EmitNode | undefined {
    return (node as { emitNode?: EmitNode; }).emitNode;
}

function getOrCreateEmitNode(node: Node): EmitNode {
    const withEmitNode = node as { emitNode?: EmitNode; };
    return withEmitNode.emitNode ??= {};
}
